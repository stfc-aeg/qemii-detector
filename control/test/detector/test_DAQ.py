...
class DAQTestFixture(object):
    ...
    def __init__(self):
        ...
        with patch("qemii.detector.QemDAQ.ParameterTree"):
...
class TestDAQ():
    ...
    def test_initialize(self, test_daq):
        ...
        assert test_daq.daq.config_files['fr'] == "qem_fr.config"
        assert test_daq.daq.config_dir == test_daq.fi_data["config_dir"]
        assert test_daq.daq.is_initialized is True
    ...
    def test_get_od_status_not_init(self, test_daq):
        ...
        assert status == {"Error": "Adapter not Initialized with references yet"}
    ...
    def test_config_odin_data(self, test_daq):
        with patch("qemii.detector.QemDAQ.ApiAdapterRequest") as mock_request:
            test_daq.daq.config_dir = "fake/dir"
            test_daq.daq.config_files["fp"] = "fp_qem.config"
    ...
    def test_acquisition_check_loop(self, test_daq):
        with patch("qemii.detector.QemDAQ.IOLoop") as mock_loop:
            ...
            test_daq.fp_data["value"][0]["hdf"]["frames_written"] = 10
    ...
    def test_start_acquisition(self, test_daq):
        with patch("qemii.detector.QemDAQ.IOLoop") as mock_loop:
            ...
            mock_loop.instance().add_callback.assert_called_with(test_daq.daq.acquisition_check_loop)
    ...
    def test_start_acquisition_needs_configure(self, test_daq):
        ...
        with patch("qemii.detector.QemDAQ.IOLoop") as mock_loop, \
             patch("qemii.detector.QemDAQ.ApiAdapterRequest") as mock_request, \
