...
class Backplane():
    ...
    def __init__(self):
        ...
        self.tpl0102 = [] #this creates a list of tpl0102 devices (potentiometers) TODO: Could this be a dict?
        self.ad7998 = []#this creates a list of ad7998 devices (Analog to Digital Converters) TODO: Could this be a dict?
        self.mcp23008 = [] #this creates a list of the GPIO devices TODO: Could this be a dict?
        ...
        self.backplane_power = True

        current_metadata = {"description": "Current being drawn by this supply", "units": "mA"}
            ...
            "VDDO": {
                "voltage": (lambda: self.voltages[0], None,
                            {"description": "Sensor main 1.8V supply",
                             "units": "V"}),
                "current": (lambda: self.currents[0], None, current_metadata)
            },
            "VDD_D18": {
                "voltage": (lambda: self.voltages[1], None,
                            {"description": "Sensor Digital 1.8V supply",
                             "units": "V"}),
                "current": (lambda: self.currents[1], None, current_metadata)
            },
            "VDD_D25": {
                "voltage": (lambda: self.voltages[2], None,
                            {"description": "Sensor Digital 2.5V supply",
                             "units": "V"}),
                "current": (lambda: self.currents[2], None, current_metadata)
            },
            "VDD_P18": {
                "voltage": (lambda: self.voltages[3], None,
                            {"description": "Sensor Programmable Gain Amplifier 1.8V supply",
                             "units": "V"}),
                "current": (lambda: self.currents[3], None, current_metadata)
            },
            "VDD_A18_PLL": {
                "voltage": (lambda: self.voltages[4], None,
                            {"description": "Sensor Analogue & Phase Lock Loop 1.8V supply",
                             "units": "V"}),
                "current": (lambda: self.currents[4], None, current_metadata)
            },
            "VDD_D18ADC": {
                "voltage": (lambda: self.voltages[5], None,
                            {"description": "Sensor Digital Analogue to Digital Converter 1.8V supply",
                             "units": "V"}),
                "current": (lambda: self.currents[5], None, current_metadata)
            },
            "VDD_D18_PLL": {
                "voltage": (lambda: self.voltages[6], None,
                            {"description": "Sensor Digital Phase Lock Loop 1.8V supply",
                             "units": "V"}),
                "current": (lambda: self.currents[6], None, current_metadata)
            },
            "VDD_A33": {
                "voltage": (lambda: self.voltages[8], None,
                            {"description": "Sensor Analogue 3.3V supply",
                             "units": "V"}),
                "current": (lambda: self.currents[8], None, current_metadata)
            },
            "VDD_D33": {
                "voltage": (lambda: self.voltages[9], None,
                            {"description": "Sensor Digital 3.3V supply",
                             "units": "V"}),
                "current": (lambda: self.currents[9], None, current_metadata)
            },
            "AUXSAMPLE_COARSE": {
                "voltage": (lambda: self.voltages[13], self.set_coarse_voltage,
                            {"description": "Sensor AUXSAMPLE COARSE VALUE input",
                             "units": "mV"}),
                "register": (lambda: self.voltages_raw[13], self.set_coarse_register,
                             {"description": "Register Value"})
            },
            "AUXSAMPLE_FINE": {
                "voltage": (lambda: self.voltages[14], self.set_fine_voltage,
                            {"description": "Sensor AUXSAMPLE FINE VALUE input",
                             "units": "uV"}),
                "register": (lambda: self.voltages_raw[14], self.set_fine_register,
                             {"description": "Register Value"})
            },
            "AUXSAMPLE": {
                "voltage": (lambda: self.voltages[15], None,
                            {"description": "Sum of coarse and fine settings"})
            },
            "VDD_RST": {
                "voltage": (lambda: self.voltages[7], self.set_vdd_rst_voltage,
                            {"description": "Sensor Reset point variable (1.8V - 3.3V) supply",
                             "units": "V"}),
                "register": (lambda: self.adjust_resistor_raw[4], self.set_vdd_rst_register_value,
                             {"description": "Register Value"}),
                "current": (lambda: self.currents[7], None, current_metadata)
            },
            "VCTRL_NEG": {
                "voltage": (lambda: self.voltages[10], None,
                            {"description": "Sensor VCTRL variable (-2V - 0V) supply",
                             "units": "V"}),
                "register": (lambda: self.voltages_raw[10], None,
                             {"description": "Register Value"}),
                "current": (lambda: self.currents[10], None, current_metadata)
            },
            "VRESET": {
                "voltage": (lambda: self.voltages[11], self.set_vreset_voltage,
                            {"description": "Sensor VRESET variable (0V - 3.3V) supply",
                             "units": "V"}),
                "register": (lambda: self.adjust_resistor_raw[5], self.set_vreset_register_value,
                             {"description": "Register Value"}),
                "current": (lambda: self.currents[11], None, current_metadata)
            },
            "VCTRL_POS": {
                "voltage": (lambda: self.voltages[12], None,
                            {"description": "Sensor VCTRL variable (0V - 3.3V) supply",
                             "units": "V"}),
                "register": (lambda: self.voltages_raw[12], None,
                             {"description": "Register Value"}),
                "current": (lambda: self.currents[12], None, current_metadata)
            },
            "VCTRL": {
                "voltage": (lambda: self.adjust_voltage[6], self.set_vctrl_voltage,
                            {"description": "calculated voltage of vctrl and vctrl set method"}),
                "register": (lambda: self.adjust_resistor_raw[6], self.set_vctrl_register_value,
                             {"description": "Register Value"})
            },
            "AUXREST": {
                "voltage": (lambda: self.adjust_voltage[0], self.set_auxreset_voltage,
                            {"description": "Sensor AUXRESET variable (0V - 3.3V) supply",
                             "units": "V"}),
                "register": (lambda: self.adjust_resistor_raw[0], self.set_auxreset_register_value,
                             {"description": "Register Value"})
            },
            "VCM": {
                "voltage": (lambda: self.adjust_voltage[1], self.set_vcm_voltage,
                            {"description": "Sensor AUXRESET variable (0V - 3.3V) supply",
                             "units": "V"}),
                "register": (lambda: self.adjust_resistor_raw[1], self.set_vcm_register_value,
                             {"description": "Register Value"})
            },
            "enable": (lambda: self.update, self.set_update,
                       {"description": "Controls I2C activity on the backplane"}),
            "backplane_power": (lambda: self.backplane_power, self.set_backplane_power),
            "clock(MHz)": (lambda: self.clock_frequency, self.set_clock_frequency,
                           {"description": "Controls the main clock Reference",
                            "units": "MHz"}),
            "dacextref": {
                "current": (self.get_dacextref, self.set_dacextref_current,
                            {"description": "Controls the DAC external current reference",
                             "units": "uA"}),
                "register": (lambda: self.adjust_resistor_raw[2], self.set_dacextref_register_value,
                             {"description": "register that controls the external reference"})
            },
            "status": {
                "level1_PG": (lambda: self.power_good[0], None,
                              {"description": "Level 1 of power supply sequence status"}),
                "level2_PG": (lambda: self.power_good[1], None,
                              {"description": "Level 2 of power supply sequence status"}),
                "level3_PG": (lambda: self.power_good[2], None,
                              {"description": "Level 3 of power supply sequence status"}),
                "level4_PG": (lambda: self.power_good[3], None,
                              {"description": "Level 4 of power supply sequence status"}),
                "level5_PG": (lambda: self.power_good[4], None,
                              {"description": "Level 5 of power supply sequence status"}),
                "level6_PG": (lambda: self.power_good[5], None,
                              {"description": "Level 6 of power supply sequence status"}),
                "level7_PG": (lambda: self.power_good[6], None,
                              {"description": "Level 7 of power supply sequence status"}),
                "level8_PG": (lambda: self.power_good[7], None,
                              {"description": "Level 8 of power supply sequence status"}),
            }
        })
    ...
    def i2c_init(self):
        ...
        logging.debug("Adjust Register Raw: %s", self.adjust_resistor_raw)
    ...
    def calc_register_from_voltage(self, voltage, par_resistor, pot_step=390, current=0.0001, offset=0):
        """Calculates the required register value for VDD_RST and VRESET when given a desired voltage
        par_resister is the resistance in parellel to the digital potentiometer.
        pot_step is the resistance increase per step of the pot

        This calculation is based off those outlined in the power supply design document"""
        val_offset = voltage + offset
        register = 1 + ((par_resistor / current) * val_offset) / ((pot_step * par_resistor) - (pot_step * (val_offset / current)))
        return int(register)
    ...
    def set_vdd_rst_voltage(self, value):
        ...
        min_voltage = -1.78
        r45_resistance = 18200  # TODO: Better variable name
        register_val = self.calc_register_from_voltage(value, r45_resistance, offset=min_voltage)
        self.tpl0102[2].set_wiper(0, register_val)
        self.adjust_resistor_raw[4] = self.tpl0102[2].get_wiper(0)
    ...
    def set_vreset_voltage(self, value):
        ...
        r55_resistance = 49900  # a resister in parallel to the i2c potentiometer

        wiper_val = self.calc_register_from_voltage(value, r55_resistance)
        self.tpl0102[2].set_wiper(1, wiper_val)
        self.adjust_resistor_raw[5] = self.tpl0102[2].get_wiper(1)
    ...
    def set_vctrl_voltage(self, value):
        #TODO: Source these magic numbers. I assume same as above?
    ...
    def calc_aux_vcm_voltage(self, value):
        """Same calculation required for AUXRESET and VCM voltages on the backplane"""
    ...
    def calc_aux_vcm_register(self, value):
        """Same calculation required for AUXRESET and VCM to calculate the register value from a voltage"""
    ...
    def set_auxreset_register_value(self, value):
